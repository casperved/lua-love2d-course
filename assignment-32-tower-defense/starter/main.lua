-- Assignment 32: Tower Defense — STARTER
-- Enemy pathfinding, waves, and drawing are all fully built for you.
-- Your job: fill in the three TODO blocks below.
--
-- Run the game now — enemies will walk the path but towers do nothing.
-- Complete TODO 1 and bullets appear. TODO 2 and they chase. TODO 3 and enemies die!
...
-- Number of trail positions stored per bullet (visual extra — already done for you)
TRAIL_LENGTH = 3
...
-- ============================================================
-- ENEMY LOGIC  (from assignment 31 — fully provided)
-- ============================================================
function spawnEnemy()
    local start = PATH[1]
    table.insert(enemies, {
        ...
        y             = start.y,
        speed         = 100,
        waypointIndex = 2,   -- next waypoint to head toward; increases as enemy advances
        hp            = 2,
        maxHp         = 2,
        dead          = false,
    })
...
function updateTowers(dt)
    for _, tower in ipairs(towers) do
        ...
        if tower.cooldown <= 0 then
            -- ================================================
            -- TODO 1: Tower targeting
            -- Pick the enemy furthest along the path (highest
            -- waypointIndex) that is within tower.range.
            -- Store it in `target`.
            --
            ...
            -- Placeholder so the code below doesn't crash before TODO 1 is done
            local target = nil
            ...
            if target then
                ...
                table.insert(bullets, {
                    ...
                    speed  = BULLET_SPEED,
                    dead   = false,
                    trail  = {},   -- stores recent positions for the visual trail
                })
            end
        end
    end
...
function updateBullets(dt)
    for _, b in ipairs(bullets) do
        -- If the target died before the bullet arrived, discard the bullet
        if b.target.dead then
            b.dead = true
        end

        if not b.dead then
            -- Visual extra: record trail position before moving (already done for you)
            table.insert(b.trail, 1, {x = b.x, y = b.y})
            if #b.trail > TRAIL_LENGTH then
                table.remove(b.trail)   -- drop the oldest trail dot
            end

            -- ================================================
            -- TODO 2: Bullet homing
            -- Every frame, recalculate the direction from this
            -- bullet to its target. Normalise the vector (divide
            -- by its length), multiply by b.speed, store in
            -- b.vx / b.vy, then move the bullet using dt.
            --
            -- Uncomment and complete the code below:
            --
            ...
            -- Hit detection (distance from bullet to its target)
            local hitDist = dist(b.x, b.y, b.target.x, b.target.y)
            if hitDist < HIT_RADIUS then
                -- ============================================
                -- TODO 3: Enemy health + death
                -- Deal damage to the target. If hp drops to
                -- 0 or below, mark it dead and award a point.
                --
                -- Uncomment and complete the code below:
                --
                ...
                b.dead = true   -- bullet is spent after hitting
            end
        end
    end

    -- Clean up dead bullets (backwards loop keeps indices stable)
...
-- ============================================================
function love.load()
    love.window.setTitle("Tower Defense — Assignment 32")
...
-- ============================================================
-- DRAWING  (all fully provided — nothing to change here)
-- ============================================================
function drawPath()
...
-- Visual extra: show range ring when mouse hovers over a tower
function drawTowers()
...
-- Visual extra: fading bullet trails
