-- Assignment 21 Solution: Saving & Loading Data
...
-- Visual extras: particle trail for the player and a pulsing enemy glow
trailPoints   = {}   -- list of {x, y, age}
TRAIL_MAX_AGE = 0.35

-- ---------------------------------------------------------------
-- SOLUTION: saveScore
-- Convert the number to a string and write it to disk.
-- ---------------------------------------------------------------
...
-- ---------------------------------------------------------------
-- SOLUTION: loadScore
-- Check whether the file exists, read it, convert back to number.
-- ---------------------------------------------------------------
function loadScore()
...
function love.load()
    ...
    -- SOLUTION: load best time on startup
    bestTime = loadScore()
...
function love.update(dt)
    ...
    if state == "playing" then
        ...
        -- VISUAL EXTRA 1: record trail positions
        ...
        -- Enemy chases player using normalised direction vector
        local dx = px - ex
        local dy = py - ey
        ...
        if dist(px, py, ex, ey) < PLAYER_RADIUS + ENEMY_RADIUS then
            -- SOLUTION: check and save a new record
...
function love.draw()
    ...
    if state == "title" then
        -- VISUAL EXTRA 2: decorative gradient circles in the background
        for i = 1, 5 do
    ...
    elseif state == "playing" then
        -- VISUAL EXTRA 1: draw player trail
        ...
        -- VISUAL EXTRA 2: pulsing glow around enemy
    ...
    elseif state == "gameover" then
        ...
        -- "New Record!" flashes for 3 seconds
