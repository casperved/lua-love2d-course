-- Assignment 26: Camera & Scrolling World — SOLUTION
-- A wider platformer with a clamped camera and collectible coins.
...
-- Visual extra: smooth camera lag
CAM_LAG = 6   -- higher = snappier; lower = floatier

camX        = 0
camXSmooth  = 0   -- the displayed camera position (lerps toward camX)
...
-- Visual extra: coin spin / bob timer
coinTimer = 0
...
function love.draw()
    ...
    -- === TODO 2 SOLUTION: Apply camera transform ===
    love.graphics.push()
    love.graphics.translate(-camXSmooth, 0)

    -- Visual extra: simple parallax sky strip drawn at half camera speed
    -- (we draw it inside push/pop but undo part of the translation manually)
    love.graphics.push()
    love.graphics.translate(camXSmooth * 0.5, 0)  -- moves at half speed
    ...
    for i = 1, #coins do
        local c = coins[i]
        if not c.collected then
            local bob = math.sin(coinTimer * 3 + i) * 3   -- oscillates ±3 px
            love.graphics.setColor(1.0, 0.85, 0.10)
            love.graphics.circle("fill", c.x, c.y + bob, c.r)
            -- Visual extra: a subtle shine dot
    ...
    -- Visual extra: eye facing direction
    ...
    love.graphics.pop()   -- end of world drawing — back to screen coords

    -- === TODO 3 SOLUTION: Draw HUD after pop ===
    ...
    -- Visual extra: a small progress bar showing how far across the level the player is
