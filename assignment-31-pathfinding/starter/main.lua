-- Assignment 31: Waypoint Pathfinding — STARTER
-- Enemies spawn at the green START dot but stand still until you fill in
-- the three TODO sections inside love.update below.

SCREEN_W = 800
SCREEN_H = 600

ENEMY_RADIUS       = 14
WAYPOINT_THRESHOLD = 20    -- px — how close counts as "arrived"
SPAWN_INTERVAL     = 2.5   -- seconds between spawns
ENEMY_SPEED        = 110   -- pixels per second

-- Winding S-shaped path — enemies travel from index 1 to index #path.
path = {
    {x=50,  y=500},
    {x=200, y=150},
    {x=400, y=450},
...
enemies    = {}   -- list of active enemy tables
lives      = 5
score      = 0
spawnTimer = 0
gameOver   = false
...
function spawnEnemy()
    local e = {
        ...
        speed         = ENEMY_SPEED,
        waypointIndex = 1,
        angle         = 0,   -- facing direction in radians (used for the arrow)
        hp            = 1,   -- health fraction 0–1 (used for the health bar)
        maxHp         = 1,
    }
...
function love.load()
    love.window.setTitle("Assignment 31 — Waypoint Pathfinding")
...
function love.update(dt)
    ...
    for i = #enemies, 1, -1 do
        ...
        -- ── TODO 1: Move toward the current waypoint ──────────────────────
        -- 1. Find dx and dy (the gap between the enemy and the target dot).
        -- 2. Find dist with math.sqrt(dx*dx + dy*dy).
        -- 3. If dist > 0, divide dx and dy by dist to get a direction,
        --    then add (dx/dist)*e.speed*dt to e.x and the same for e.y.
        -- 4. Also store the facing angle: e.angle = math.atan2(ny, nx)
        --    where nx = dx/dist and ny = dy/dist.
        --
        ...
        -- ── TODO 2: Advance to the next waypoint when close enough ─────────
        -- Use the same dist variable from TODO 1.
        -- If dist < WAYPOINT_THRESHOLD, add 1 to e.waypointIndex.
        --
        -- if dist < WAYPOINT_THRESHOLD then
        --     e.waypointIndex = e.waypointIndex + 1
        -- end

        -- ── TODO 3: Handle enemy reaching the exit ─────────────────────────
        -- If e.waypointIndex > #path the enemy has walked past the last dot.
        -- Remove it from the enemies table and subtract 1 from lives.
        --
...
function drawEnemies()
    for _, e in ipairs(enemies) do
        ...
        -- Direction arrow (points the way the enemy is travelling)
        ...
        -- Health bar (floats above the enemy)
